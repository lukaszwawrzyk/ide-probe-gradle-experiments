...
RUN echo "deb https://repo.scala-sbt.org/scalasbt/debian all main" | tee -a /etc/apt/sources.list.d/sbt.list \
    ...
    && apt-get -qq update \
    && apt-get -qq -o=Dpkg::Use-Pty=0 install libxtst6 libx11-6 libxrender1 xvfb openssh-server \
        sbt libssl-dev pkg-config x11-apps vim imagemagick zip openjdk-11-jre openjdk-11-jdk
